...
function NoNeedToRoll:loadMap(savegame)
	...
	self:loadNeedsRollingFruitTypes()
	self:updateNeedsRollingFruitTypes()
end

function NoNeedToRoll:loadNeedsRollingFruitTypes()
	for i = 1, #g_fruitTypeManager.fruitTypes do
		...
		if fruitType.needsRolling == true then
			NoNeedToRoll.ROLLING_FRUIT_TYPES[fruitType.index] = true
		end
	end
end

function NoNeedToRoll:updateNeedsRollingFruitTypes()
	for i = 1, #g_fruitTypeManager.fruitTypes do
		local fruitType = g_fruitTypeManager.fruitTypes[i]

		if NoNeedToRoll.ROLLING_FRUIT_TYPES[fruitType.index] == true then
			fruitType.needsRolling = FSMissionInfo.rollingRequiredEnabled
		end
	end
end

addModEventListener(NoNeedToRoll)
...
FSBaseMission.setRollingRequiredEnabled = function (self, isEnabled, noEventSend)
	if isEnabled ~= self.missionInfo.rollingRequiredEnabled then
		...
		NoNeedToRoll:updateNeedsRollingFruitTypes()
...
local function onFrameOpen(self, superFunc, element)
	...
	if not NoNeedToRoll.GUI_CREATED then
		local checkRollingRequired = self.checkPlowingRequired:clone(self.boxLayout)

		checkRollingRequired.elements[4]:setText(self.l10n:getText('setting_rollingRequired', NoNeedToRoll.MOD_NAME))
		checkRollingRequired.elements[6]:setText(self.l10n:getText('toolTip_rollingRequired', NoNeedToRoll.MOD_NAME))
...
local function getRollerFactor(self, superFunc, startWorldX, startWorldZ, widthWorldX, widthWorldZ, heightWorldX, heightWorldZ)
...
FSDensityMapUtil.getRollerFactor = Utils.overwrittenFunction(FSDensityMapUtil.getRollerFactor, getRollerFactor)
